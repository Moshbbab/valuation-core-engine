...
from core.intelligence_kernel.market_normalization import MarketNormalizer  # noqa: E402
from core.intelligence_kernel.comparable_adjustments import ComparableAdjuster  # noqa: E402
from core.intelligence_kernel.weighting_engine import WeightingEngine  # noqa: E402
from core.intelligence_kernel.fair_value_hierarchy import FairValueHierarchyClassifier  # noqa: E402
from core.intelligence_kernel.uncertainty_model import UncertaintyModel  # noqa: E402
from core.intelligence_kernel.confidence_index import ConfidenceIndexCalculator  # noqa: E402
...
class ValuationPipeline:
    ...
    def run(self, subject: Dict, comparables: List[Dict]) -> Dict:
        ...
        market_params = {"annual_appreciation": 0.04}
        ...
        market_activity = {"transaction_count": 18, "expected_volume": 15}
        ...
        for comp in norm_result.normalized_data:
            try:
                adj_result = self.adjuster.calculate_adjustments(subject, comp, market_params)
        ...
        approach_values = {"market": market_value, "income": market_value * 1.02, "cost": market_value * 0.98}

        data_quality_scores = {"market": 0.95, "income": 0.80, "cost": 0.70}

        weighting_result = self.weighting_engine.calculate_weighted_value(
            approach_values, data_quality_scores, subject["property_type"]
        )
        ...
        hierarchy_result = self.hierarchy_classifier.classify(data_sources, assumptions, market_activity)
        ...
        uncertainty_result = self.uncertainty_model.calculate_uncertainty(
            weighting_result.final_value, adjusted_comparables, confidence_level=0.95
        )
        ...
        disclosure_data = {"total_inputs": 15, "disclosed_inputs": 13}

        vci_result = self.vci_calculator.calculate_vci(
            data_sources, adjusted_comparables, adjustment_results, market_activity, disclosure_data
        )
