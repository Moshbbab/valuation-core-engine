...
class ComparableAdjuster:
    ...
    def calculate_adjustments(
        ...
        if 'sqft' in subject and 'sqft' in comparable:
            adj_size, details = self._calculate_size_adjustment(
                subject['sqft'],
                comparable['sqft']
            )
        ...
        if 'bedrooms' in subject and 'bedrooms' in comparable:
            adj_bed, details = self._calculate_bedroom_adjustment(
                subject['bedrooms'],
                comparable['bedrooms']
            )
        ...
        if 'bathrooms' in subject and 'bathrooms' in comparable:
            adj_bath, details = self._calculate_bathroom_adjustment(
                subject['bathrooms'],
        ...
        if 'year_built' in subject and 'year_built' in comparable:
            adj_age, details = self._calculate_age_adjustment(
                subject['year_built'],
                comparable['year_built']
            )
        ...
        if 'location_score' in subject and 'location_score' in comparable:
            adj_loc, details = self._calculate_location_adjustment(
                subject['location_score'],
                comparable['location_score']
            )
        ...
        if 'sale_date' in comparable:
            adj_time, details = self._calculate_time_adjustment(
                ...
                market_params.get('annual_appreciation', 0.03)
            )
    ...
    def _calculate_bedroom_adjustment(
        self,
        subject_beds: int,
        comp_beds: int
    ) -> tuple:
    ...
    def _calculate_bathroom_adjustment(
        self,
        subject_baths: float,
        comp_baths: float
    ) -> tuple:
