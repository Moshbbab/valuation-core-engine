...
class ConfidenceIndexCalculator:
    ...
    def _calculate_data_quality(self, data_sources: List[Dict], comparables: List[Dict]) -> float:
        """
    ...
    def _assess_source_reliability(self, data_sources: List[Dict]) -> float:
        ...
        reliability_scores = {"mls": 95, "public_records": 90, "api": 85, "scraper": 70, "manual": 60}
    ...
    def _calculate_comparable_strength(self, comparables: List[Dict], adjustments: List[Dict]) -> float:
        """
    ...
    def _interpret_vci(self, vci: float) -> str:
        ...
        if vci >= 90:
            ...
            return (
                "Very low confidence. Valuation is highly uncertain and should not be relied upon"
    ...
    def _generate_recommendations(self, component_scores: Dict[str, float]) -> List[str]:
        ...
        if component_scores["market_liquidity"] < 70:
            recommendations.append(
                "Consider expanding geographic search area or time period to capture more market activity."
            )
