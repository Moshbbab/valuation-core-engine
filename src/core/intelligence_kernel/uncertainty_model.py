...
class UncertaintyModel:
    ...
    def calculate_uncertainty(
        self, value: float, comparables: List[Dict], confidence_level: float = 0.95
    ) -> UncertaintyResult:
        """
    ...
    def calculate_value_range(
        self, comparables: List[Dict], percentile_low: float = 0.25, percentile_high: float = 0.75
    ) -> Tuple[float, float]:
        """
    ...
    def assess_model_stability(self, comparables: List[Dict], value: float) -> Dict:
        """
        ...
        if len(prices) < 2:
            return {"stable": False, "reason": "Insufficient comparables"}
