...
class TestCalculateAdjustments:
    ...
    def test_identical_comparable_zero_size_adjustment(
        self, adjuster, subject, identical_comparable
    ):
        result = adjuster.calculate_adjustments(subject, identical_comparable)
        assert result.adjustments['size'] == pytest.approx(0.0)

    def test_identical_comparable_zero_bedroom_adjustment(
        self, adjuster, subject, identical_comparable
    ):
        result = adjuster.calculate_adjustments(subject, identical_comparable)
        assert result.adjustments['bedrooms'] == pytest.approx(0.0)

    def test_identical_comparable_zero_bathroom_adjustment(
        self, adjuster, subject, identical_comparable
    ):
        result = adjuster.calculate_adjustments(subject, identical_comparable)
        assert result.adjustments['bathrooms'] == pytest.approx(0.0)

    def test_identical_comparable_zero_age_adjustment(
        self, adjuster, subject, identical_comparable
    ):
        result = adjuster.calculate_adjustments(subject, identical_comparable)
        assert result.adjustments['age'] == pytest.approx(0.0)

    def test_identical_comparable_zero_location_adjustment(
        self, adjuster, subject, identical_comparable
    ):
        result = adjuster.calculate_adjustments(subject, identical_comparable)
    ...
    def test_total_adjustment_is_sum_of_parts(
        self, adjuster, subject, different_comparable
    ):
        result = adjuster.calculate_adjustments(subject, different_comparable)
...
class TestAdjustmentDirections:
    def test_larger_subject_positive_size_adjustment(self, adjuster, subject):
        smaller_comp = {
            'id': 'c', 'price': 1_000_000, 'sqft': 2_000,
            'sale_date': datetime.now().isoformat(),
        }
    ...
    def test_smaller_subject_negative_size_adjustment(self, adjuster, subject):
        larger_comp = {
            'id': 'c', 'price': 1_000_000, 'sqft': 3_000,
            'sale_date': datetime.now().isoformat(),
        }
    ...
    def test_more_bedrooms_positive_adjustment(self, adjuster, subject):
        fewer_beds_comp = {
            'id': 'c', 'price': 1_000_000, 'sqft': subject['sqft'],
            'bedrooms': 2, 'sale_date': datetime.now().isoformat(),
        }
    ...
    def test_newer_subject_positive_age_adjustment(self, adjuster, subject):
        older_comp = {
            'id': 'c', 'price': 1_000_000, 'sqft': subject['sqft'],
            'year_built': 2005, 'sale_date': datetime.now().isoformat(),
        }
    ...
    def test_time_adjustment_positive_for_older_sale(self, adjuster, subject):
        ...
        result = adjuster.calculate_adjustments(
            subject, old_sale_comp, {'annual_appreciation': 0.04}
        )
...
class TestExcessiveAdjustment:
    def test_raises_when_adjustment_exceeds_threshold(self, adjuster, subject):
        ...
        with pytest.raises(ExcessiveAdjustmentError):
            adjuster.calculate_adjustments(
                subject, extreme_comp, {'annual_appreciation': 0.04}
            )
...
class TestAdjustmentGrid:
    def test_grid_contains_all_comparables(
        self, adjuster, subject, different_comparable, identical_comparable
    ):
        grid = adjuster.generate_adjustment_grid(
            subject, [different_comparable, identical_comparable]
        )
    ...
    def test_valid_comp_marked_valid(
        self, adjuster, subject, identical_comparable
    ):
