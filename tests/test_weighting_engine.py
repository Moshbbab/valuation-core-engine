...
import pytest
from core.intelligence_kernel.weighting_engine import WeightingEngine, WeightingResult, PropertyType
...
class TestCalculateWeightedValue:
    def test_returns_weighting_result(self, engine, equal_approach_values, uniform_quality):
        result = engine.calculate_weighted_value(
            equal_approach_values, uniform_quality, 'residential'
        )
    ...
    def test_weights_sum_to_one(self, engine, residential_values, uniform_quality):
        result = engine.calculate_weighted_value(
            residential_values, uniform_quality, 'residential'
        )
...
class TestPropertyTypeWeights:
    ...
    def test_commercial_income_weight_dominant(self, engine, equal_approach_values, uniform_quality):
        result = engine.calculate_weighted_value(
            equal_approach_values, uniform_quality, 'commercial'
        )
    ...
    def test_unknown_type_defaults_to_residential(
        self, engine, equal_approach_values, uniform_quality
    ):
        result_res = engine.calculate_weighted_value(
            equal_approach_values, uniform_quality, 'residential'
        )
        result_unk = engine.calculate_weighted_value(
            equal_approach_values, uniform_quality, 'unknown_type'
        )
...
class TestQualityAdjustedWeights:
    def test_lower_quality_income_reduces_income_weight(
        self, engine, residential_values
    ):
        high_income_quality = {'market': 1.0, 'income': 1.0, 'cost': 1.0}
        low_income_quality = {'market': 1.0, 'income': 0.3, 'cost': 1.0}

        result_high = engine.calculate_weighted_value(
            residential_values, high_income_quality, 'residential'
        )
        result_low = engine.calculate_weighted_value(
            residential_values, low_income_quality, 'residential'
        )
        assert result_low.weights['income'] < result_high.weights['income']

    def test_weights_still_sum_to_one_with_varied_quality(
        self, engine, residential_values
    ):
        quality = {'market': 0.9, 'income': 0.6, 'cost': 0.4}
        result = engine.calculate_weighted_value(
            residential_values, quality, 'residential'
        )
